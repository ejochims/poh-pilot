...
A third flow (**Mediafly content search**) is included as a Level 1 reference implementation: the rep asks for sales materials and the agent searches Mediafly's Launchpad API (`GET /items/search` with `IsAISearch=true`) and returns direct links to the matching assets, with Mediafly staying the single source of truth (no content duplicated into Salesforce). See `Mediafly-Agent-Action-Walkthrough.md` for the full walkthrough, including the **authentication decision** used — a single dedicated Mediafly service-account identity, chosen because the public Mediafly API supports no OAuth/delegated per-rep token flow — the API constraint that keeps this at "find & link" rather than answering from inside assets, and the roadmap to per-user scoping.

> **Confirmed by Mediafly (July 2026).** Mediafly reviewed this reference implementation and confirmed the API path: `GET /items/search` with `IsAISearch=true` is the right endpoint; viewer links stay permission-enforced on open; and the public API authorizes at the **service-account level, not per user** — so search *results* reflect the integration account's access. Mediafly recommends the **API route for a live July 27 demo** and **MCP + OAuth as the governed production path** for per-user, permission-scoped search. See the walkthrough's *Authentication Decision* and *Mediafly's Recommendation and Roadmap* sections.
...
>
> **Setting it up? Start with the step-by-step runbook: [`Mediafly-Setup-Guide.md`](Mediafly-Setup-Guide.md).** It walks the whole install in order (what to get from Mediafly, credentials, config, code, agent wiring, testing, troubleshooting). Read [`Mediafly-Agent-Action-Walkthrough.md`](Mediafly-Agent-Action-Walkthrough.md) for the design rationale and API details.
